...
### 3.6 メール送信機能（v2.1 新機能）

録音ファイルをメールで送信する機能です。

#### 機能概要
- 録音ファイル一覧から任意のファイルを選択してメール送信
...
| 拡張子 | MIMEタイプ | 説明 |
|-------|-----------|------|
| m4a | audio/mp4 | 標準録音フォーマット |
| wav | audio/wav | WAV形式 |
| mp3 | audio/mpeg | MP3形式 |
| aac | audio/aac | AAC形式 |
...
| 項目 | 内容 |
|-----|------|
| 件名 | 【MedicalRecorder】{ファイル名（拡張子なし）} |
| 本文 | MedicalRecorderから録音ファイルを送信します。<br>ファイル名: {ファイル名} |
| 添付 | 選択した音声ファイル |

#### エラーハンドリング
- メールアカウント未設定時: アラート表示で設定を促す
- 送信失敗時: エラーメッセージを表示

#### 録音ファイル一覧
- Documentsディレクトリ内の音声ファイル（m4a, wav）を一覧表示
- 表示情報: ファイル名、録音時間、ファイルサイズ、作成日時
- 作成日時の降順でソート
- スワイプによるファイル削除機能
...
├── AudioImporter.swift           # 音声インポート
└── MailManager.swift             # メール送信管理（新）
```
...
- **新機能:** メール送信機能
  - 録音ファイル一覧表示
  - 録音ファイルをメールに添付して送信
  - MFMailComposeViewController による標準メールUI
  - 複数音声フォーマット対応（m4a, wav, mp3, aac）
- **新機能:** 録音ファイル管理
  - 録音ファイル一覧表示（ファイル名、録音時間、サイズ、日時）
  - スワイプによるファイル削除
- **新規ファイル:** MailManager.swift
